...
class KafkaStreamsTopologyConfiguration {
    ...
    @Bean("imageCollectorTopologyProperties")
    fun streamsProperties(kafkaAppProperties: KafkaAppProperties): Properties {
        // env
        val bootstrapServers = kafkaAppProperties.bootstrapServers
        val schemaRegistryUrl = kafkaAppProperties.schemaRegistryUrl
        val applicationName = "image-collector-app"
    ...
    @Bean("imageCollectorTopology")
    fun imageCollectorTopology(imageCollectorTopologyProperties: Properties, objectMapper: ObjectMapper): KafkaStreams {
        val upsertTopic = "example-stream.product.upsert"
        ...
        val imageCollectorStream: KStream<String, GenericRecord> = builder.stream(upsertTopic)

        imageCollectorStream.mapValues { value ->
            val jsonNode = objectMapper.readTree(value.get("fullDocument").toString())
            val productId = jsonNode.get("_id").toString()
            val imageUrl = jsonNode.get("imageUrl").toString()
        ...
        // topology 구성
        val streams = KafkaStreams(builder.build(), imageCollectorTopologyProperties)
    ...
    fun mockUploadS3(productId:String, imageUrl: String): String {
        Thread.sleep(10) // 잠깐 딜레이주고

        val url = "http://수집됐다.com/" + productId + "/" + LocalDateTime.now() + "/" + UUID.randomUUID()
        return url
    }
}